...
.EXAMPLE
.\run-instrumentation-demo.ps1 -TargetFolders ".\core\instrumentor-test\src\main\java\com\example\instrumentor\happens"
.EXAMPLE
...
if (-not $SkipBuildAndTest) {
    ...
    java -cp .\core\instrumentor-test\target\instrumentor-test-1.0-SNAPSHOT.jar com.example.instrumentor.happens.before.SyncTest
    if ($LASTEXITCODE -ne 0) {
