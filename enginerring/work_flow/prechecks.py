...
def print_disclaimer():
    ...
    print_color(" This workflow uses the sample code located in: ", Colors.YELLOW, end="")
    print_color("core/instrumentor-test\n", Colors.CYAN)
    print_color(" [The Pre-set Bug]", Colors.YELLOW)
    print_color(" There is an intentional concurrency bug in 'Test 8: Event-Driven Aggregation'.", Colors.YELLOW)
    print_color(" Symptom: The final output array is [0, 0, 0] instead of the expected [500, 1000, 1500].", Colors.YELLOW)
    print_color(" Root Cause: A Happens-Before violation where the main thread reads the results", Colors.YELLOW)
    print_color("             before the async EventBus finishes writing them.\n", Colors.YELLOW)
    print_color(" [Disclaimer]", Colors.YELLOW)
    print_color(" Modern LLMs are incredibly powerful. Even without this project's methodology,", Colors.YELLOW)
    print_color(" they can easily spot this specific bug just by reading the static source code.", Colors.YELLOW)
    print_color(" Therefore, this Quickstart is NOT meant to prove the superiority of this tool", Colors.YELLOW)
    print_color(" on simple bugs. Instead, it serves as a sandbox to demonstrate the GENERAL WORKFLOW:", Colors.YELLOW)
