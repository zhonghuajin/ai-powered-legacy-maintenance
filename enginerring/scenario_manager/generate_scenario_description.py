...
Step: Analyze final-output-calltree.md & execution_flow_with_code.md -> build prompt -> call LLM -> output scenario_schema.json
...
def generate_scenario_description(work_dir: str, proj_path: str = None, move_outputs: bool = True):
    ...
    prompt = f"""You are a concurrency analysis expert. Based on the provided thread call tree and detailed execution flow with source code, generate a JSON DATA INSTANCE that conforms to the provided scenario_schema.json.

**Data Sources Description**:
1. **Call Tree (`final-output-calltree.md`)**: Reflects the runtime appearance order of files, sorted by thread within the current scenario, along with their intra-file function call relationships.
2. **Execution Flow with Code (`execution_flow_with_code.md`)**: Reflects the runtime appearance order of functions, sorted and presented by thread within the current scenario, along with their source code.
...
- Infer sub-processes, threads, components, interactions, and concurrency models from the call tree and the actual executed source code.
