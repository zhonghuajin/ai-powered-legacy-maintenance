...
Step: Analyze final-output-calltree.json -> build prompt -> call LLM -> output scenario_schema.json
...
def generate_scenario_description(work_dir: str, proj_path: str = None, move_outputs: bool = True):
    """
    After log denoising, utilize final-output-calltree.json and the scenario_schema.json
    ...
    calltree_file = os.path.join(work_dir, 'final-output-calltree.json')
    ...
    prompt = f"""You are a concurrency analysis expert. Based on the provided thread call tree (from final-output-calltree.json), generate a JSON DATA INSTANCE that conforms to the provided scenario_schema.json.
