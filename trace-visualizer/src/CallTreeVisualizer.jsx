  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inCodeBlock) {
      if (line.trim().startsWith('```')) {
        inCodeBlock = false;
        if (currentCodeNode) currentCodeNode.source = trimCommonIndentation(codeLines);
        codeLines = [];
    ...
    if (threadMatch) {
      const rootNode = { file: 'virtual', method: 'Thread Files', source: '', calls: [] };
      currentThread = {
    ...
    if (fileMatch) {
      ...
      currentFileNode = { file: filePath, method: fileName, source: '', calls: [], isFileNode: true };
      if (currentThread) currentThread.call_tree.calls.push(currentFileNode);
    ...
    if (methodMatch) {
      const indentLevel = Math.floor(methodMatch[1].length / 4);
      const node = {
        file: currentFileNode ? currentFileNode.file : '',
        method: methodMatch[2],
        paramCount: parseInt(methodMatch[3], 10),
        source: '',
        calls: []
      };
    ...
    if (extMatch) {
      const indentLevel = Math.floor(extMatch[1].length / 4);
      const node = {
        file: currentFileNode ? currentFileNode.file : '',
        method: extMatch[2],
        isExternal: true,
    ...
    if (line.trim().startsWith('```')) {
      inCodeBlock = true;
      codeLines = [];
      currentCodeNode = nodeStack.length > 0 ? nodeStack[nodeStack.length - 1].node : null;
    }
  }

  threads.forEach(t => {
    if (t.call_tree && t.call_tree.method === 'Thread Files' && t.call_tree.calls.length === 1) {
    ...
    <div className="mx-auto max-w-4xl space-y-6">
      <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center shadow-sm">
        ...
        <p className="mt-3 text-sm leading-6 text-slate-600 max-w-xl mx-auto">
          Upload the generated <strong>final-output-calltree.md</strong> file.
        </p>
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-slate-600">
        <div className="space-y-2">
          ...
          <p className="text-xs">
            Explicit calls within the same file (like <code>$this-&gt;foo()</code>) are fully expanded as nested tree nodes.
          </p>
        </div>
        <div className="space-y-2">
          <p className="font-medium text-slate-800">🔌 External/Unknown Calls (Isolated)</p>
          <p className="text-xs">
            Calls to external classes are marked as <code>[External]</code> and act as leaf nodes.
          </p>
        </div>
      </div>
    </div>
...
function SourcePanel({ node }) {
  ...
  const handleCopy = async () => {
    try {
      ...
      await navigator.clipboard.writeText(prefix + (node?.source || ''));
      setCopied(true);
      ...
      <div className="flex items-start gap-2 justify-between">
        <div className="min-w-0 flex-1">
          <div className="flex items-start gap-1.5">
            {nodeIcon}
            <div className="font-mono text-xs font-semibold text-slate-900 break-all" title={node.method}>
              {node.method}
            </div>
          </div>
          {node.paramCount !== undefined && (
            <div className="mt-1 pl-5 text-2xs text-slate-500">
              Params: <span className="font-mono font-medium">{node.paramCount}</span>
            </div>
          )}
        </div>
        <span className={`shrink-0 rounded-full px-2 py-0.5 text-2xs font-medium ${badgeColor}`}>
          {badgeText}
        </span>
      </div>
    </button>
  ...
  return (
    <div className="relative">
      ...
      {depth > 0 && (
        <span
          aria-hidden
          className="pointer-events-none absolute -left-3 top-4 h-px w-3 bg-slate-300"
        />
      )}
      ...
      <div className="flex items-start gap-1.5 py-0.5">
        <button
          ...
          className={classNames(
            ...
            hasChildren
              ? 'border-slate-300 bg-white text-slate-600 hover:border-indigo-300 hover:bg-indigo-50 hover:text-indigo-600'
        ...
        <div className="min-w-0 flex-1">
          <NodeMeta node={node} isSelected={isSelected} onSelect={() => setSelectedNode(node)} />
        </div>
      </div>
      ...
      <AnimatePresence initial={false}>
        {expanded && hasChildren && (
          ...
          >
            {/* 这条竖线把所有子节点串起来 */}
            <div className="relative ml-3 border-l border-slate-300 pl-3">
...
export default function CallTreeVisualizer() {
  ...
  const handleFileUpload = useCallback(async (event) => {
    ...
    try {
      ...
      if (!parsedData || !Array.isArray(parsedData.threads) || parsedData.threads.length === 0) {
        throw new Error('Could not find any valid Thread blocks.');
      }
      setData(parsedData);
      setError('');
      ...
      if (firstThread?.call_tree) {
        setSelectedNode(firstThread.call_tree.method === 'Thread Files' && firstThread.call_tree.calls.length > 0
          ? firstThread.call_tree.calls[0]
          : firstThread.call_tree);
      } else {
  ...
  const filteredThreads = useMemo(() => {
    ...
    return threads.filter(t =>
      [t?.name, t?.call_tree?.method].map(s => String(s || '').toLowerCase()).some(s => s.includes(q))
    );
  }, [threads, searchText]);

  const activeThreadStats = useMemo(() => {
    if (!activeThread?.call_tree) return { methods: 0, files: new Set(), externals: 0 };
  ...
  const handleSwitchThread = (thread) => {
    ...
    if (thread?.call_tree) {
      setSelectedNode(thread.call_tree.method === 'Thread Files' && thread.call_tree.calls.length > 0
        ? thread.call_tree.calls[0]
        : thread.call_tree);
    } else {
      setSelectedNode(null);
    }
      ...
      <main className="mx-auto max-w-screen-2xl px-4 py-6 md:px-6 lg:px-8">
        ...
        {!data ? (
          ...
          <div className="flex h-[calc(100vh-12rem)] gap-6">
                ...
                <div className={classNames(
                  ...
                  {!sidebarCollapsed && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-semibold text-sm text-slate-900">Call Tree</span>
                    </div>
                  )}
                  <button
                ...
                {!sidebarCollapsed && (
                  <div className="flex-1 overflow-auto p-3">
                    {activeThread?.call_tree ? (
                      ...
                      <div className="text-sm text-slate-500 p-4">No call tree data for this thread</div>
                    )}
                  </div>
                )}
              </div>
            ...
            {/* ---- 主区域：节点详情与源代码 ---- */}
            <section className="flex-1 min-w-0 overflow-y-auto space-y-6">
              {activeThread ? (
                <>
                  {/* 线程概览卡片 */}
                  <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h2 className="text-xl font-bold text-slate-900">{activeThread.name}</h2>
                      <span className="text-sm text-slate-500">Order #{activeThread.order}</span>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <SummaryCard icon={FolderTree} label="Files" value={formatCount(activeThreadStats.files.size)} />
                      <SummaryCard icon={ListTree} label="Methods" value={formatCount(activeThreadStats.methods)} />
                      <SummaryCard icon={ExternalLink} label="Externals" value={formatCount(activeThreadStats.externals)} theme="amber" />
                    </div>
                  </div>
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    {selectedNode ? (
                      <div className="space-y-4">
                        ...
                        {selectedNode.file && selectedNode.file !== 'virtual' && !selectedNode.isFileNode && (
                          <div>
                            ...
                            <div className="mt-1 break-all text-sm text-slate-700 font-mono">{selectedNode.file}</div>
                          </div>
                        )}
                      ...
                      <div className="rounded-xl bg-slate-50 p-4 text-sm text-slate-500">
                        Select a node from the tree in the sidebar to view its details and source code.
                      </div>
                    )}
                  </div>
                </>
